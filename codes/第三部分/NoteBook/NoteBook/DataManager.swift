...
class DataManager: NSObject {
    //创建一个数据库操作对象属性
    static var sqlHnadle:Connection?
    //标记是否已经打开数据库
    ...
    class func saveGroup(name:String){
        ...
        let _ = try! sqlHnadle?.run(insert)
    }
    //提供一个获取所有分组数据的类方法
    class func getGroupData()->[String]{
        ...
        let groupTable = Table("group")
        //进行查询数据操作
        for group in try! sqlHnadle!.prepare(groupTable) {
            //遍历查询到的数据 赋值进结果数组中
    ...
    class func getNote(group:String)->[NoteModel]{
        ...
        let ownGroup = Expression<String?>("ownGroup")
        for note in try! sqlHnadle!.prepare(noteTable.filter(ownGroup.like(group))) {
            //遍历查询到的数据 赋值进结果数组中
            let ownGroup = Expression<String?>("ownGroup")
            let body = Expression<String?>("body")
    ...
    class func updateNote(note:NoteModel){
        ...
        let title = Expression<String?>("title")
        let time = Expression<String?>("time")
        let _ = try? sqlHnadle?.run(alice.update(ownGroup <- note.group, body <- note.body, title <- note.title, time <- note.time))
    }
    ...
    class func deleteNote(note:NoteModel){
        ...
        let nId = Expression<Int64>("id")
        let alice = noteTable.filter(nId == Int64(note.noteId!))
        let _ = try? sqlHnadle?.run(alice.delete())
    }
    ...
    class func deleteGroup(name:String){
        ...
        //再删除分组
        let _ = try? sqlHnadle?.run(gAlice.delete())
    }
...
.userDomainMask, true).first!
        ...
        //执行建表
        let _ = try? sqlHnadle?.run(groupTable.create(block: { (t) in
        ...
        let time = Expression<String?>("time")
        //执行建表
        let _ = try? sqlHnadle?.run(noteTable.create(block: { (t) in
